...
# ========= get 1st clustering annotation from cola
rlb <- readr::read_rds("/cluster/home/yjliu_jh/projects/leu_j/data/rlb_exp_3000.rds")
clu3000 <- get_classes(rlb, k = 11)
clu3000$sample_id <- rownames(clu3000)
batch <- left_join(batch, clu3000[, c("sample_id", "class")])
batch$class <- paste0("G", batch$class)
