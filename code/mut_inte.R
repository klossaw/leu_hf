...
for(i in 1:length(samples)){
  tryCatch({
    temp <- read.maf(glue("{mafloc}/{samples[i]}/{samples[i]}.HaplotypeCaller.maf"))
    temp_3 <- temp@data[, c("Hugo_Symbol", "Chromosome", "Start_Position", "End_Position",
                            "Reference_Allele", "Tumor_Seq_Allele1", "Tumor_Seq_Allele2",
                            "Variant_Classification", "Variant_Type", "Consequence", "PUBMED", "FILTER",
                            "HGVSc", "EXON", "INTRON", "t_depth", "t_ref_count", "t_alt_count", "BIOTYPE",
...
for(i in 1:length(samples)){
  tryCatch({
    temp <- read.maf(glue("{mafloc}/{samples[i]}/{samples[i]}.rnaindel.maf"))
    temp_3 <- temp@data[, c("Hugo_Symbol", "Chromosome", "Start_Position", "End_Position",
                            "Reference_Allele", "Tumor_Seq_Allele1", "Tumor_Seq_Allele2",
                            "Variant_Classification", "Variant_Type", "Consequence", "PUBMED", "FILTER",
                            "HGVSc", "EXON", "INTRON", "t_depth", "t_ref_count", "t_alt_count", "BIOTYPE",
...
readr::write_rds(ri, "/cluster/home/yjliu_jh/projects/leu_j/data/rnaindel.rds")

vs2s <- list()
for(i in 1:length(samples)){
  tryCatch({
    temp <- read.maf(glue("{mafloc}/{samples[i]}/{samples[i]}.snp.varscan2.maf"))
    temp_3 <- temp@data[, c("Hugo_Symbol", "Chromosome", "Start_Position", "End_Position",
                            "Reference_Allele", "Tumor_Seq_Allele1", "Tumor_Seq_Allele2",
                            "Variant_Classification", "Variant_Type", "Consequence", "PUBMED", "FILTER",
                            "HGVSc", "EXON", "INTRON", "t_depth", "t_ref_count", "t_alt_count", "BIOTYPE",
                            "SIFT", "PolyPhen", "IMPACT", "CLIN_SIG", "Existing_variation", "Protein_position")]
    colnames(temp_3)[22] <- "VEP_Impact"
    vs2s[[samples[i]]] <- temp_3
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
readr::write_rds(vs2s, "/cluster/home/yjliu_jh/projects/leu_j/data/snp.varscan2.rds")

vs2i <- list()
for(i in 1:length(samples)){
  tryCatch({
    temp <- read.maf(glue("{mafloc}/{samples[i]}/{samples[i]}.indel.varscan2.maf"))
    temp_3 <- temp@data[, c("Hugo_Symbol", "Chromosome", "Start_Position", "End_Position",
                            "Reference_Allele", "Tumor_Seq_Allele1", "Tumor_Seq_Allele2",
                            "Variant_Classification", "Variant_Type", "Consequence", "PUBMED", "FILTER",
                            "HGVSc", "EXON", "INTRON", "t_depth", "t_ref_count", "t_alt_count", "BIOTYPE",
                            "SIFT", "PolyPhen", "IMPACT", "CLIN_SIG", "Existing_variation", "Protein_position")]
    colnames(temp_3)[22] <- "VEP_Impact"
    vs2i[[samples[i]]] <- temp_3
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
readr::write_rds(vs2i, "/cluster/home/yjliu_jh/projects/leu_j/data/indel.varscan2.rds")
